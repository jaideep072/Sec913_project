...
async def list_users(token: Optional[str] = Header(None)):
    return await mongo_spring_request("GET", "/admin/users", token=token)

@router.put("/users/{user_id}/role")
async def set_role(user_id: str, body: Dict[str, Any] = Body(...), token: Optional[str] = Header(None)):
    return await mongo_spring_request("PUT", f"/admin/users/{user_id}/role", json=body, token=token)

@router.put("/users/{user_id}/status")
async def set_status(user_id: str, body: Dict[str, Any] = Body(...), token: Optional[str] = Header(None)):
    return await mongo_spring_request("PUT", f"/admin/users/{user_id}/status", json=body, token=token)

@router.delete("/users/{user_id}")
async def delete_user(user_id: str, token: Optional[str] = Header(None)):
    return await mongo_spring_request("DELETE", f"/admin/users/{user_id}", token=token)
