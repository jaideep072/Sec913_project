...
echo ===================================================
echo.
echo [1/5] Starting Spring Boot PostgreSQL Backend (Port 8001)...
start "Spring Boot PostgreSQL Backend" cmd /k "cd Backend\coreservices_AKS && mvnw spring-boot:run"

echo [2/5] Starting Spring Boot MongoDB Backend (Port 8020)...
start "Spring Boot MongoDB Backend" cmd /k "cd Backend\coreservices_mongo && mvnw spring-boot:run"

echo [3/5] Starting Node.js Reviews Backend (Port 8002)...
start "Node.js Reviews Backend" cmd /k "cd Backend\node_service && npm start"

echo [4/5] Starting FastAPI Gateway (Port 8000)...
